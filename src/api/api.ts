...
export default class Api {
  ...
  // 请求列表，在这里添加相应接口
  apiList: ApiList = {
    login: { url: "/user/login", fetchType: "json", method: "post" },
    logout: { url: "/user/logout", fetchType: "json", method: "post" },
    getUserInfo: { url: "/user/getUserInfo", fetchType: "json", method: "post" },

    baseInfoAdd: { url: "/customers/baseInfo/add", fetchType: "json", method: "post" },
    baseInfoUpdate: { url: "/customers/baseInfo/update", fetchType: "json", method: "post" },
    baseInfoDelete: { url: "/customers/baseInfo/delete", fetchType: "json", method: "post" }
  };
  ...
  request = (options: Options) => this.fetch(options)
    .then((response: any) => {
      ...
      if (response.data.result && response.data.result.resultCode === 3) {
        ...
        return Promise.reject({ success: false, message: response.data.result.resultMessage });
      }
  ...
  fetch = (options: Options) => {
    ...
    const headers = { token: window.localStorage.getItem("token"), ...options.headers };
    ...
    switch (method.toLowerCase()) {
      ...
      case "delete":
        return this.service.delete(url, { data: cloneData, headers });
      case "post":
